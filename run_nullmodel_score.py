...
############### NOTE ###############
# edits wrt run_fc_score.py are annotated
# all outputs are silenced -- only logs / predictions
####################################
...
for tpdx, tup in enumerate(all_param_lst[offset:]):
    ...
    ############### EDIT ###############
    ## replace all text with null strings
    for key, df in input_data_d.items():
        new_df = df
        new_df['text'] = ['']*len(df)
        input_data_d[key] = new_df
    ####################################
    ...
    for epoch in range(1, these_params['epochs']+1):
        ...
        tr_d.update({'model_name': model_name, 'epoch': epoch, 'split': 'train',
                     ...
                     'null_probs': tr_probs, 'preds': tr_preds, 'losses': tr_losses})
        val_d = {}
        val_d.update({'model_name': model_name, 'epoch': epoch, 'split': 'val',
                     'ID': val_IDs, 'text': val_texts, 'labels': val_true,
                     'null_probs': val_probs, 'preds': val_preds, 'losses': val_losses})
        te_d = {}
        te_d.update({'model_name': model_name, 'epoch': epoch, 'split': 'test',
                     'ID': te_IDs, 'text': te_texts, 'labels': te_true,
                     'null_probs': te_probs, 'preds': te_preds, 'losses': te_losses})
        
        MODEL_LEDGER = pd.concat([ MODEL_LEDGER, pd.DataFrame.from_dict(tr_d),
                                  pd.DataFrame.from_dict(val_d), pd.DataFrame.from_dict(te_d) ])
        
        ############### EDIT ###############
        ledgerLocation = f"ledgers/{model_name}.csv"
        if os.path.exists(ledgerLocation):
            in_LEDGER = pd.read_csv(ledgerLocation)

            in_pre = in_LEDGER[in_LEDGER['epoch']<epoch]
            in_edf = in_LEDGER[in_LEDGER['epoch']==epoch]
            if 'null_probs' in in_edf.columns:
                    in_edf = in_edf.drop('null_probs', axis=1)
            in_post = in_LEDGER[in_LEDGER['epoch']>epoch]
            in_join = pd.merge(in_edf, MODEL_LEDGER[['ID', 'epoch', 'null_probs']],
                                  on=['ID', 'epoch'], how='left')
            
            out_LEDGER = pd.concat([ in_pre, in_join, in_post ])

            # let's overwrite the ledger each epoch to be safe
            out_LEDGER.to_csv(f'ledgers/{model_name}.csv', index=False)
        ####################################
        
        ############### EDIT ###############
        ## not writing anything for null model -- also no patience to ensure 15 epochs (we're not trying to "learn")
        # # WRITE LOGS / OUTPUTS
